...
impl NotificationService {
    ...
    pub async fn send(
        &self,
        mut stream: impl Stream<Item = Result<SendRequest, Status>> + Send + Unpin + 'static,
        ...
        tokio::spawn(async move {
            while let Some(Ok(req)) = stream.next().await {
                ...
                let res = match req.msg {
                    ...
                    None => Err(Status::invalid_argument("msg is required")),
                };

                tx.send(res).await.expect("Should send ok");
            }
        });
